...
app.post('/api/checkout', async (req: Request, res: Response) => {
    try {
        ...
        const statement = await db.prepare('INSERT INTO order_items (order_id, product_id, quantity, price) VALUES (?, ?, ?, ?)');
        for (const item of cart) {
            await statement.run(orderId, item.product_id, item.quantity, item.price);
        }
        ...
        res.status(200).json({ message: "Operacja zakończona pomyślnie", orderId });
    } catch (e) {
        console.error(e);
...
app.post('/api/products', async (req: Request, res: Response) => {
    try {
        ...
        const result = await db.run(
            'INSERT INTO products (id,title, price, description, category, image, stock) VALUES (?,?, ?, ?, ?, ?, ?)',
            [id, title, price, description, category, image, stock]
        );
...
app.post('/api/register', async (req: Request, res: Response) => {
    try {
        ...
        if (err.code === 'SQLITE_CONSTRAINT') return res.status(409).json({ error: "Nazwa użytkownika jest zajęta" });
        res.status(500).json({ error: "Błąd serwera" });
    }
});
...
app.delete('/api/reviews/:id', async (req: Request, res: Response) => {
    try {
        ...
        if (!review) return res.status(404).json({ error: "Nie znaleziono opinii" });
...
app.get('/sync', async (req, res) => {
    try {
        ...
        const statement = await db.prepare(`INSERT OR REPLACE INTO products (id,title, price, description, category, image, stock) VALUES (?,?, ?, ?, ?, ?, ?)`);
        products.forEach(item => {
            statement.run(item.id, item.title, item.price, item.description, item.category, item.image, 50);
        });
        statement.finalize();
