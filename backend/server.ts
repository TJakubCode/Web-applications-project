  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const cart = await db.all(
        "SELECT c.quantity, p.id as product_id, p.price FROM cart c JOIN products p ON c.product_id = p.id WHERE c.username = ?",
        [username],
      );
...
app.post("/api/cart", authenticateJWT, async (req: Request, res: Response) => {
  try {
    ...
    if (existing) {
      await db.run("UPDATE cart SET quantity = quantity + ? WHERE id = ?", [
        quantity,
        existing.id,
      ]);
    } else {
...
app.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await db.run(
        "INSERT INTO reviews (username, product_id, content) VALUES (?, ?, ?)",
...
app.get("/sync", async (req, res) => {
  try {
    ...
    const statement = await db.prepare(
      `INSERT OR REPLACE INTO products (id,title, price, description, category, image, stock) VALUES (?,?, ?, ?, ?, ?, ?)`,
    );
